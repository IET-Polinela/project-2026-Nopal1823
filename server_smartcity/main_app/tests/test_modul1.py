...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # Arrange: login sebagai warga biasa (bukan admin) via session auth
        self.client.login(username='warga_test', password='Password123!')

        # Act: warga biasa mencoba mengakses halaman daftar laporan admin
        response = self.client.get(reverse('report_list'))

        # Assert: ditolak dan di-redirect (302) karena bukan admin
        self.assertEqual(
            response.status_code,
            302,
            "Warga biasa seharusnya di-redirect (302) saat mengakses "
            "halaman khusus admin, bukan diberi akses langsung."
        )