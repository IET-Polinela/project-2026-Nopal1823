...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_02_laporan_saya_menampilkan_nama_asli(self):
        ...
        PENJELASAN TEKNIS:
            Serializer proyek ini TIDAK memiliki field 'reporter_name'
            terpisah. Field 'reporter' SELALU mengembalikan "Warga Anonim"
            (hardcoded di get_reporter(), terlepas dari siapa pemiliknya).
            Kepemilikan asli ditandai lewat field boolean 'is_owner', yang
            dihitung dengan membandingkan obj.reporter.pk == request.user.pk.
            Jadi pada tab 'my_reports', yang diverifikasi adalah is_owner
            == True untuk setiap laporan milik sendiri.
        """
        self.client.force_authenticate(user=self.warga_a)
        ...
        # Verifikasi: is_owner True untuk semua laporan di tab 'my_reports'
        # (karena tab ini hanya mengembalikan laporan milik user yang login)
        for laporan in results:
            self.assertTrue(
                laporan['is_owner'],
                f"Pada tab 'my_reports', is_owner seharusnya True untuk "
                f"laporan milik sendiri, tetapi bernilai '{laporan['is_owner']}'"
            )
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Act: Warga A mencoba GET detail draf milik Warga B
        url = f'/api/report/{self.draft_milik_b.id}/'
        response = self.client.get(url)

        # Assert: sistem berpura-pura draf tidak ada (404), bukan 403
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik orang lain seharusnya tidak ditemukan (404), "
            "bukan ditolak (403), demi menyembunyikan eksistensinya."
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        """
        # Arrange: autentikasi sebagai Warga A, siapkan payload modifikasi
        self.client.force_authenticate(user=self.warga_a)
        judul_asli = self.draft_milik_b.title
        payload = {
        ...
        # Act: Warga A mengirim PUT ke draf milik Warga B
        url = f'/api/report/{self.draft_milik_b.id}/'
        response = self.client.put(url, payload, format='json')

        # Assert: modifikasi ditolak dengan 404, dan data asli tidak berubah
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Modifikasi draf milik orang lain seharusnya ditolak dengan 404."
        )
        self.draft_milik_b.refresh_from_db()
        self.assertEqual(
            self.draft_milik_b.title,
            judul_asli,
            "Judul asli draf tidak boleh berubah sama sekali."
        )