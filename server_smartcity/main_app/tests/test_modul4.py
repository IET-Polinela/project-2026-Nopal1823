...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            ...
            'description': 'Lampu jalan sudah mati selama 3 hari.',
        ...
        # Assert: berhasil dibuat dengan 201
        self.assertEqual(
            response.status_code,
            status.HTTP_201_CREATED,
            "Pembuatan laporan dengan data lengkap seharusnya berhasil (201)"
        )

        # Assert: data tersimpan dan field reporter otomatis terisi
        laporan = Report.objects.get(title='Lampu Jalan Mati di Gang Sempit')
        self.assertEqual(
            laporan.reporter,
            self.warga,
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        """
        # Arrange: payload tanpa field title
        url = reverse('report-list')
        payload = {
            'category': 'Fasilitas Umum',
        ...
        # Assert: ditolak dengan 400 dan pesan error menyebut field title
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Laporan tanpa judul seharusnya ditolak (400 Bad Request)"
        )
        self.assertIn(
            'title',
            response.data,
            "Pesan error harus menyebutkan field 'title' sebagai penyebabnya."
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        HASIL YANG DIHARAPKAN:
            Sistem menolak input dan mengembalikan HTTP 400 Bad Request.
        """
        # Arrange: payload tanpa field description
        url = reverse('report-list')
        payload = {
        ...
        # Assert: ditolak dengan 400 dan pesan error menyebut field description
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Laporan tanpa deskripsi seharusnya ditolak (400 Bad Request)"
        )
        self.assertIn(
            'description',
            response.data,
            "Pesan error harus menyebutkan field 'description' sebagai penyebabnya."
        )
