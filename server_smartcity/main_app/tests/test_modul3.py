...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        """
        # Arrange: autentikasi sebagai pemilik laporan, siapkan payload edit
        self.client.force_authenticate(user=self.warga)
        judul_asli = self.laporan_reported.title

        url = f'/api/report/{self.laporan_reported.pk}/'
        payload = {
            'title': 'Judul yang Dicoba Diubah',
        ...
        # Assert: ditolak dengan 403, dan data asli tetap aman
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Modifikasi laporan berstatus REPORTED seharusnya ditolak (403)"
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        """
        # Arrange: autentikasi sebagai pemilik laporan yang sudah RESOLVED
        self.client.force_authenticate(user=self.warga)
        judul_asli = self.laporan_resolved.title

        url = f'/api/report/{self.laporan_resolved.pk}/'
        payload = {
            'title': 'Mencoba Mengubah Laporan Selesai',
        ...
        # Assert: ditolak dengan 403, data tetap terkunci
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan RESOLVED bersifat read-only, modifikasi harus ditolak (403)"
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        # Act: admin mengirim POST untuk mengubah status REPORTED -> VERIFIED
        url = reverse('update_status', kwargs={'pk': self.laporan_reported.pk})
        response = self.client.post(url, {'status': 'VERIFIED'})

        # Assert: request berhasil diproses (redirect setelah POST)
        self.assertIn(
            response.status_code,
            [200, 302],
            "Perubahan status oleh admin seharusnya berhasil (200/302)"
        )
        self.laporan_reported.refresh_from_db()
        self.assertEqual(
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        """
        # Arrange: laporan_reported berstatus REPORTED (dari setUp)

        # Act: ambil daftar status yang diizinkan dari posisi REPORTED
        allowed_statuses = self.laporan_reported.get_allowed_statuses()

        # Assert: RESOLVED tidak ada di daftar transisi yang diizinkan
        self.assertNotIn(
            'RESOLVED',
            allowed_statuses,
            "Dari status REPORTED, transisi langsung ke RESOLVED "
            "seharusnya tidak diizinkan (harus lewat VERIFIED dan "
            "IN_PROGRESS terlebih dahulu)."
        )

        # Assert tambahan: VERIFIED adalah satu-satunya transisi maju yang valid
        self.assertIn(
            'VERIFIED',
            allowed_statuses,
            "VERIFIED seharusnya menjadi transisi maju yang valid dari REPORTED."
        )
        self.assertTrue(
            self.laporan_reported.is_transition_allowed('VERIFIED'),
            "is_transition_allowed('VERIFIED') harus mengembalikan True."
        )
        self.assertFalse(
            self.laporan_reported.is_transition_allowed('RESOLVED'),
            "is_transition_allowed('RESOLVED') harus mengembalikan False."
        )